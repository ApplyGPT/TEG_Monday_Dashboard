...
def refresh_calendly_database(config):
    ...
    try:
        ...
        if not user_uri:
            print("❌ Calendly API did not return user URI")
            return False
        ...
        if event_types_response.status_code != 200:
            print(f"❌ Failed to get Calendly event types: {event_types_response.status_code} - {event_types_response.text[:200]}")
            return False
        
        event_types_data = event_types_response.json()
        event_types = event_types_data.get('collection', [])
        
        # Include: Burki, Intro Call, and Design Review event types (same logic as pages/database_refresh.py)
        ...
        if not teg_event_types:
            print("❌ No TEG event types found ('TEG - Let's Chat', TEG Introductory Call, or Design Review 30min links)")
            return False
        ...
        for event_type, source in teg_event_types:
            ...
            while page_count < 100:
                ...
                if events_response.status_code != 200:
                    print(f"   ⚠️ Failed to get events for {event_name}: {events_response.status_code}")
                    break
                ...
                for event in events:
                    ...
                    all_events.append({**event, "source": source})
